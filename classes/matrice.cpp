...
// Constructeur pour une matrice de taille lignes x colonnes
matrice::matrice(size_t lignes, size_t colonnes) : lignes(lignes), colonnes(colonnes) {
    mat.resize(lignes, std::vector<complexe>(colonnes, complexe(0, 0)));  // Initialiser avec des zéros
}
...
void matrice::setElement(size_t i, size_t j, double valeur) {
    if (i >= lignes || j >= colonnes) {
        throw std::out_of_range("Indices hors des limites de la matrice.");
    }
    mat[i][j] = valeur;
}
...
matrice matrice::dephasage(double xi) {
    return matrice(1, 0, 0, std::exp(complexe(0, xi)));
}
